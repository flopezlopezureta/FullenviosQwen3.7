    ...
    if (files && files.length > 0) {
      ...
      try {
          ...
          if (file.size > 50 * 1024 * 1024) {
               setError("Imagen demasiado grande (máximo 50MB).");
               return;
          }
          ...
          try {
              ...
              try {
                  ...
                  console.warn("[ReturnModal] Library compression failed, using canvas fallback", err);
              ...
              console.error("Critical error processing image in ReturnModal", err);
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {photosBase64.map((photo, index) => (
                                <div key={index} className="relative aspect-square">
                                    <img src={photo} alt={`Prueba de devolución ${index + 1}`} className="rounded-md w-full h-full object-cover" />
